...
const create = asyncHandler(async (req, res) => {
  ...
  if (req.user.role !== 'doctor') throw forbidden('Only doctors can create prescriptions');
  ...
  const doc = await Prescription.create({ patientId: String(patientId), doctorId, medications, notes, createdBy: req.user.id });
...
const getById = asyncHandler(async (req, res) => {
  ...
  if (req.user.role === 'patient') {
    ...
    const doctorId = getDoctorIdFromUser(req.user);
    if (doc.doctorId !== doctorId) throw forbidden('Doctors may only read prescriptions they authored');
...
const list = asyncHandler(async (req, res) => {
  ...
  const items = await Prescription.find(q).sort({ createdAt: -1 }).limit(100).lean();
